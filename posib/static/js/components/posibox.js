/* Posib - CMSimple
 * by flatLand!
 * JS File - /posib/static/js/components/posibox.js
 */

/*jshint nonstandard: true, browser: true, boss: true */
/*global jQuery */

( function( $ ) {
    "use strict";

    $.fn.posibox = function( args ) {
        var defaultConfig = {
                cssPrefix: "posibox-",
                overlayOpacity: 0.66,
                loaderGifURL: "data:image/gif;base64,",
                zIndexBase: 99999
            },
            config = $.extend( defaultConfig, args ),
            imagesToOpen = [],
            posiboxIsOpen = false,
            currentIndex = 0,
            posiboxClicked = function( e ) {
                e.preventDefault();
                if( jQuery( this ).data( 'gallery-images' ) ) {
                    imagesToOpen = eval( '(' + unescape( jQuery( this ).data( 'gallery-images' ) ) + ')' );
                } else if( jQuery( this ).data( 'gallery-image' ) ) {
                    imagesToOpen = [ jQuery( this ).data( 'gallery-image' ) ];
                } else {
                    imagesToOpen = [ jQuery( this ).attr( 'href' ) ];
                }
                openPosibox();
            },
            openPosibox = function() {
                jQuery( window ).scrollTop( 0 );
                jQuery( 'div.' + config.cssPrefix + 'overlay' ).fadeTo( 'fast', config.overlayOpacity );
                jQuery( '.' + config.cssPrefix + 'next, .' + config.cssPrefix + 'prev' ).toggle( imagesToOpen.length > 1 );
                jQuery( 'div.' + config.cssPrefix + 'container' ).css( {
                    width: 50,
                    height: 50,
                    top: ( $( window ).height() - 50 ) / 2,
                    left: "50%",
                    marginLeft: -25
                } ).fadeIn( 'normal', function() {
                    currentIndex = 0;
                    posiboxIsOpen = true;
                    loadImage( imagesToOpen[ currentIndex ] );
                } );
            },
            closePosibox = function( e ) {
                e.preventDefault();
                if( posiboxIsOpen ) {
                    jQuery( 'div.' + config.cssPrefix + 'container' ).fadeOut( 'fast' );
                    jQuery( 'div.' + config.cssPrefix + 'overlay' ).fadeOut( 'normal', function() {
                        posiboxIsOpen = false;
                        jQuery( 'div.' + config.cssPrefix + 'container img' ).remove();
                    } );
                }
            },
            loadImage = function( imageInfo ) {
                var img = jQuery( '<img src="' + imageInfo + '" />' ).css( {
                    display: "inline-block",
                    borderRadius: "5px",
                    cursor: "pointer"
                } ).load( function() {
                    jQuery( this ).hide().appendTo( 'div.' + config.cssPrefix + 'container' );
                    jQuery( 'div.' + config.cssPrefix + 'container' ).animate( {
                        width: jQuery( this ).width(),
                        height: jQuery( this ).height(),
                        top: ( jQuery( this ).height() < $( window ).height() ) ? ( ( $( window ).height() - jQuery( this ).height() ) / 2 ) : 10,
                        marginLeft: -( jQuery( this ).width() / 2 ),
                        padding: '5px'
                    }, 'fast', function() {
                        jQuery( img ).on( 'click', imagesToOpen.length > 1 ? nextImage : closePosibox ).fadeIn( 'fast' );
                    } );
                } );
            },
            nextImage = function() {
                jQuery( 'div.' + config.cssPrefix + 'container img' ).fadeOut( 'fast', function() {
                    jQuery( 'div.' + config.cssPrefix + 'container img' ).remove();
                    currentIndex = ( currentIndex + 1 < imagesToOpen.length ) ? ++currentIndex : 0;
                    loadImage( imagesToOpen[ currentIndex ] );
                } );
            },
            prevImage = function() {
                jQuery( 'div.' + config.cssPrefix + 'container img' ).fadeOut( 'fast', function() {
                    jQuery( 'div.' + config.cssPrefix + 'container img' ).remove();
                    currentIndex = ( currentIndex === 0 ) ? ( imagesToOpen.length - 1 ) : --currentIndex ;
                    loadImage( imagesToOpen[ currentIndex ] );
                } );
            };

        if( $( 'body > div.' + config.cssPrefix + 'overlay' ).size() === 0 ) {
            $( '<div class="' + config.cssPrefix + 'overlay' + '"></div>' ).css( {
                width: $( document ).outerWidth( true ),
                height: $( document ).outerHeight( true ),
                position: "absolute",
                top: 0,
                left: 0,
                background: "black",
                zIndex: config.zIndexBase
            } ).hide().appendTo( 'body' ).on( 'click', closePosibox );
            $( '<div class="' + config.cssPrefix + 'container' + '"></div>' ).css( {
                width: 50,
                height: 50,
                position: "absolute",
                zIndex: config.zIndexBase + 1,
                top: ( $( window ).height() - 50 ) / 2,
                left: "50%",
                marginLeft: -25,
                background: "white url( " + config.loaderGifURL + " ) center center no-repeat",
                borderRadius: 5,
                boxShadow: "0 0 10px rgba( 0, 0, 0, 0.95 )"
            } ).hide().appendTo( 'body' );
            $( '<span class="' + config.cssPrefix + 'close' + '">&times;</span>' ).css( {
                display: "block",
                width: 19,
                height: 19,
                position: "absolute",
                top: -10,
                right: -10,
                color: "white",
                background: "black",
                border: "2px solid white",
                borderRadius: "50%",
                boxShadow: "0 0 5px rgba( 0, 0, 0, 0.95 )",
                font: "bold 19px/16px Verdana, sans-serif",
                textAlign: "center",
                cursor: "pointer"
            } ).appendTo( 'div.' + config.cssPrefix + 'container' ).on( 'click', closePosibox );
            $( '<span class="' + config.cssPrefix + 'next' + '">&rsaquo;</span>' ).css( {
                display: "block",
                width: 19,
                height: 19,
                position: "absolute",
                top: "50%",
                marginTop: -10,
                right: -10,
                color: "white",
                background: "black",
                border: "2px solid white",
                borderRadius: "50%",
                boxShadow: "0 0 5px rgba( 0, 0, 0, 0.95 )",
                font: "bold 19px/16px Verdana, sans-serif",
                textAlign: "center",
                cursor: "pointer"
            } ).hide().appendTo( 'div.' + config.cssPrefix + 'container' ).on( 'click', nextImage );
            $( '<span class="' + config.cssPrefix + 'prev' + '">&lsaquo;</span>' ).css( {
                display: "block",
                width: 19,
                height: 19,
                position: "absolute",
                top: "50%",
                marginTop: -10,
                left: -10,
                color: "white",
                background: "black",
                border: "2px solid white",
                borderRadius: "50%",
                boxShadow: "0 0 5px rgba( 0, 0, 0, 0.95 )",
                font: "bold 19px/16px Verdana, sans-serif",
                textAlign: "center",
                cursor: "pointer"
            } ).hide().appendTo( 'div.' + config.cssPrefix + 'container' ).on( 'click', prevImage );
        }

        return this.each( function() {
            jQuery( this ).on( 'click', posiboxClicked );
        } );
    };
} )( jQuery );
